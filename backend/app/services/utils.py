...
def count_tokens_simple(text: str) -> int:
    """Ước lượng số token cho tiếng Việt (số từ * 1.3)"""
    words = text.split()
...
def load_data():
    ...
    try:
        cursor.execute("""
            ...
            ORDER BY d.doc_id, ch.chapter_id, a.article_id, cl.clause_number
        """)
...
def format_chunk_unified(doc_title, chapter_title, article_title, clause_content):
    """
    Format chunk theo chuẩn thống nhất cho cả BM25 và FAISS.
...
def text_splitter(data):
    ...
    for row in data:
        ...
        clause_content = row['clause_content'].strip().lower()
        
        if not clause_content:
            continue
        
        chunk = format_chunk_unified(doc_title, chapter_title, article_title, clause_content)
